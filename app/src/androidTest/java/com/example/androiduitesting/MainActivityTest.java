...
// You can also use anything() in place of is(instanceOf(String.class))
    ...
    public void testBackButton() {
        // Add a unique city to the list
        onView(withId(R.id.button_add)).perform(click());
        onView(withId(R.id.editText_name)).perform(ViewActions.typeText("Edmonton"));
        onView(withId(R.id.button_confirm)).perform(click());

        // Click on the newly added city to open CityDetailActivity
        onData(is("Edmonton"))
                .inAdapterView(withId(R.id.city_list))
                .perform(click());

        // Now click the back button in the detail activity
        onView(withId(R.id.back_button)).perform(click());

        // Verify we have returned to MainActivity by checking the city list is visible
        onView(withId(R.id.city_list)).check(matches(isDisplayed()));

}
    @Test
    public void testClickCity_opensDetailActivity() {
        // 1. Add a unique city to the list (so test doesn't depend on others)
        onView(withId(R.id.button_add)).perform(click());
        onView(withId(R.id.editText_name)).perform(ViewActions.typeText("Edmonton"));
        onView(withId(R.id.button_confirm)).perform(click());

        // 2. Click on the newly added city in the ListView
        onData(is("Edmonton"))
                .inAdapterView(withId(R.id.city_list))
                .perform(click());

        // 3. Verify that the detail activity is displayed by checking its views
        onView(withId(R.id.back_button)).check(matches(isDisplayed()));
        onView(withId(R.id.city_detail_text)).check(matches(withText("Edmonton")));
    }
}
